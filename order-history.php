		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-total last-item">Recommendation</th>
				</tr>
			</thead><!-- /thead -->
...
?>
			<?php $pid =  $row['proid'];?>
			<form method="post">
				<input type="hidden" name="prodid" id="prodid" value="<?php echo $row['proid'];?>">
			</form>
				<tr>
					<td><?php echo $cnt;?></td>
					<td class="cart-image">
 ...
 <a href="javascript:void(0);" onClick="popUpWindow('track-order.php?oid=<?php echo htmlentities($row['orderid']);?>');" title="Track order">
					Track</td>
					<td>
					<?php $ret = mysqli_query($con,"SELECT * FROM recommended WHERE productId='$pid' and userId='".$_SESSION['id']."'");
$num=mysqli_num_rows($ret);
if($num>0)
{ ?>
	<p>Recommendation already recorded</p>
<?php }else{?>

 <a href="recm.php?pid=<?php echo htmlentities($row['proid']);?>&status=Same" title="Recommendation">
					Same genre || <a href="recm.php?pid=<?php echo htmlentities($row['proid']);?>&status=Different" title="Recommendation">Different genre
					</td>
				</tr>
<?php $cnt=$cnt+1;} } ?>
...
function doAction(action, message) {
    ...
    type: 'POST',
    url: 'recm.php',                
    data: {actionrec:actionrec,productid:productid},
